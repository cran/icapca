...
double AIR_ica_MLCf15_j2(/*@unused@*/ const unsigned int unused, const double *w_parameters, double *dw_temp, void **constants){
	 ...
	 */
	{
		...
		f+=(m-ngc)*n*(log_2pi)/2.0;		/* Gaussian */
		{
			unsigned int j;
			
			double a0=(n-1.0)/n;
			double a1=(double)n/(n+1.0);
			double g1=0;
			double g2=0;
			double g3=0;
			double g4=0;
			{
				AIR_Error errcode;
				g1=AIR_dgamma((n-1.0)/2.0 + 1.0/n, &errcode);
				g2=AIR_dgamma((n-1.0-gc)/2.0, &errcode);
				g3=AIR_dgamma((n-1.0)/2.0, &errcode);
				g4=AIR_dgamma((n-1.0-gc)/2.0+1.0/n, &errcode);
			}
			double cplus=(n+1.0)*(n-1.0-gc-2*a0)*g1*g2/((n+a1)*(n-1.0-gc-1.0)*g3*g4);
			...
			for(j=0;j<n;j++){
				 ...
				 */
				{
					...
					temp*=(n/(n-1.0));
					//f+=0.5*c*temp*n/((n-1)*(1.0-((1.0+temp)/n)));
					f+=0.5*cplus*(1.0+a1*(1.0/n))*temp*pow((1.0-temp/n),-a0);
				}
			}
		}
		if(dw_temp==NULL){
